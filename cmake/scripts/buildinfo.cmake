...
# 
# this module expects
# WSH_SOURCE_DIR - main CMAKE_SOURCE_DIR
# WSH_DST_DIR - main CMAKE_BINARY_DIR
# WSH_BUILD_TYPE
# WSH_BUILD_PLATFORM
#
# example usage:
# cmake -DWSH_SOURCE_DIR=. -DWSH_DST_DIR=build -DWSH_BUILD_TYPE=Debug -DWSH_BUILD_PLATFORM=Darwin.appleclang -P scripts/buildinfo.cmake
#
# Its main output variables are SOL_VERSION_BUILDINFO and SOL_VERSION_PRERELEASE

if (NOT WSH_BUILD_TYPE)
	set(WSH_BUILD_TYPE "unknown")
endif()

if (NOT WSH_BUILD_PLATFORM)
	set(WSH_BUILD_PLATFORM "unknown")
endif()
...
if (EXISTS ${WSH_SOURCE_DIR}/prerelease.txt)
	file(READ ${WSH_SOURCE_DIR}/prerelease.txt SOL_VERSION_PRERELEASE)
...
if (EXISTS ${WSH_SOURCE_DIR}/commit_hash.txt)
	file(READ ${WSH_SOURCE_DIR}/commit_hash.txt SOL_COMMIT_HASH)
	string(STRIP ${SOL_COMMIT_HASH} SOL_COMMIT_HASH)
else()
	execute_process(
		COMMAND git --git-dir=${WSH_SOURCE_DIR}/.git --work-tree=${WSH_SOURCE_DIR} rev-parse --short=8 HEAD
		OUTPUT_VARIABLE SOL_COMMIT_HASH OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET
	)
	execute_process(
		COMMAND git --git-dir=${WSH_SOURCE_DIR}/.git --work-tree=${WSH_SOURCE_DIR} diff HEAD --shortstat
...
set(SOl_VERSION_PLATFORM WSH_BUILD_PLATFORM)
set(SOL_VERSION_BUILDINFO "commit.${SOL_COMMIT_HASH}.${WSH_BUILD_PLATFORM}")

set(TMPFILE "${WSH_DST_DIR}/BuildInfo.h.tmp")
set(OUTFILE "${WSH_DST_DIR}/BuildInfo.h")

configure_file("${WSH_BUILDINFO_IN}" "${TMPFILE}")

include("${WSH_CMAKE_DIR}/WshUtils.cmake")
