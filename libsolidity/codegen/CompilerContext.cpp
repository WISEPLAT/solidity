...
{
	wsh::AssemblyItem retTag = pushNewTag();
	CompilerUtils(*this).moveIntoStack(_inArgs);

	*this << lowLevelFunctionTag(_name, _inArgs, _outArgs, _generator);

	appendJump(wsh::AssemblyItem::JumpType::IntoFunction);
	adjustStackOffset(int(_outArgs) - 1 - _inArgs);
	*this << retTag.tag();
}

wsh::AssemblyItem CompilerContext::lowLevelFunctionTag(
	string const& _name,
	unsigned _inArgs,
	unsigned _outArgs,
	function<void(CompilerContext&)> const& _generator
)
{
	...
	{
		wsh::AssemblyItem tag = newTag().pushTag();
...
void CompilerContext::appendMissingLowLevelFunctions()
{
	while (!m_lowLevelFunctionGenerationQueue.empty())
	{
		...
		*this << m_lowLevelFunctions.at(name).tag();
		...
		appendJump(wsh::AssemblyItem::JumpType::OutOfFunction);
...
wsh::Assembly const& CompilerContext::compiledContract(const ContractDefinition& _contract) const
{
...
wsh::AssemblyItem CompilerContext::functionEntryLabel(Declaration const& _declaration)
{
	return m_functionCompilationQueue.entryLabel(_declaration, *this);
}

wsh::AssemblyItem CompilerContext::functionEntryLabelIfExists(Declaration const& _declaration) const
{
...
CompilerContext& CompilerContext::appendJump(wsh::AssemblyItem::JumpType _jumpType)
{
	wsh::AssemblyItem item(Instruction::JUMP);
...
CompilerContext& CompilerContext::appendConditionalInvalid()
{
	*this << Instruction::ISZERO;
	wsh::AssemblyItem afterTag = appendConditionalJump();
...
wsh::AssemblyItem CompilerContext::FunctionCompilationQueue::entryLabel(
...
)
{
	...
	{
		wsh::AssemblyItem tag(_context.newTag());
...
wsh::AssemblyItem CompilerContext::FunctionCompilationQueue::entryLabelIfExists(Declaration const& _declaration) const
{
	auto res = m_entryLabels.find(&_declaration);
	return res == m_entryLabels.end() ? wsh::AssemblyItem(wsh::UndefinedItem) : res->second.tag();
}
