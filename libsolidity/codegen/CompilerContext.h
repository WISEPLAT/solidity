...
public:
	explicit CompilerContext(CompilerContext* _runtimeContext = nullptr):
		m_asm(std::make_shared<wsh::Assembly>()),
	...
	void setCompiledContracts(std::map<ContractDefinition const*, wsh::Assembly const*> const& _contracts) { m_compiledContracts = _contracts; }
	wsh::Assembly const& compiledContract(ContractDefinition const& _contract) const;
	...
	wsh::AssemblyItem functionEntryLabel(Declaration const& _declaration);
	/// @returns the entry label of the given function. Might return an AssemblyItem of type
	/// UndefinedItem if it does not exist yet.
	wsh::AssemblyItem functionEntryLabelIfExists(Declaration const& _declaration) const;
	...
	/// Note that the generator should not assume that objects are still alive when it is called,
	/// unless they are guaranteed to be alive for the whole run of the compiler (AST nodes, for example).
	wsh::AssemblyItem lowLevelFunctionTag(
		std::string const& _name,
		unsigned _inArgs,
		unsigned _outArgs,
		std::function<void(CompilerContext&)> const& _generator
	...
	wsh::AssemblyItem appendConditionalJump() { return m_asm->appendJumpI().tag(); }
	/// Appends a JUMPI instruction to @a _tag
	CompilerContext& appendConditionalJumpTo(wsh::AssemblyItem const& _tag) { m_asm->appendJumpI(_tag); return *this; }
	/// Appends a JUMP to a new tag and @returns the tag
	wsh::AssemblyItem appendJumpToNew() { return m_asm->appendJump().tag(); }
	/// Appends a JUMP to a tag already on the stack
	CompilerContext& appendJump(wsh::AssemblyItem::JumpType _jumpType = wsh::AssemblyItem::JumpType::Ordinary);
	...
	CompilerContext& appendJumpTo(wsh::AssemblyItem const& _tag) { m_asm->appendJump(_tag); return *this; }
	/// Appends pushing of a new tag and @returns the new tag.
	wsh::AssemblyItem pushNewTag() { return m_asm->append(m_asm->newPushTag()).tag(); }
	/// @returns a new tag without pushing any opcodes or data
	wsh::AssemblyItem newTag() { return m_asm->newTag(); }
	/// @returns a new tag identified by name.
	wsh::AssemblyItem namedTag(std::string const& _name) { return m_asm->namedTag(_name); }
	/// Adds a subroutine to the code (in the data section) and pushes its size (via a tag)
	/// on the stack. @returns the pushsub assembly item.
	wsh::AssemblyItem addSubroutine(wsh::AssemblyPointer const& _assembly) { return m_asm->appendSubroutine(_assembly); }
	...
	wsh::AssemblyItem appendData(bytes const& _data) { return m_asm->append(_data); }
	...
	CompilerContext& operator<<(wsh::AssemblyItem const& _item) { m_asm->append(_item); return *this; }
	...
	wsh::LinkerObject const& assembledObject() const { return m_asm->assemble(); }
	wsh::LinkerObject const& assembledRuntimeObject(size_t _subIndex) const { return m_asm->sub(_subIndex).assemble(); }
...
private:
	...
	struct FunctionCompilationQueue
	{
		...
		wsh::AssemblyItem entryLabel(Declaration const& _declaration, CompilerContext& _context);
		/// @returns the entry label of the given function. Might return an AssemblyItem of type
		/// UndefinedItem if it does not exist yet.
		wsh::AssemblyItem entryLabelIfExists(Declaration const& _declaration) const;
		...
		std::map<Declaration const*, wsh::AssemblyItem> m_entryLabels;
	...
	wsh::AssemblyPointer m_asm;
	/// Activated experimental features.
	std::set<ExperimentalFeature> m_experimentalFeatures;
	/// Other already compiled contracts to be used in contract creation calls.
	std::map<ContractDefinition const*, wsh::Assembly const*> m_compiledContracts;
	...
	std::map<std::string, wsh::AssemblyItem> m_lowLevelFunctions;
