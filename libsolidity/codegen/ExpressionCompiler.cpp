...
bool ExpressionCompiler::visit(Conditional const& _condition)
{
	...
	wsh::AssemblyItem trueTag = m_context.appendConditionalJump();
	...
	wsh::AssemblyItem endTag = m_context.appendJumpToNew();
	m_context << trueTag;
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case FunctionType::Kind::Internal:
		{
			...
			wsh::AssemblyItem returnLabel = m_context.pushNewTag();
			for (unsigned i = 0; i < arguments.size(); ++i)
			{
				arguments[i]->accept(*this);
			...
			m_context.appendJump(wsh::AssemblyItem::JumpType::IntoFunction);
		...
		case FunctionType::Kind::Creation:
		{
			...
			m_context.callLowLevelFunction(
				...
				[contract](CompilerContext& _context)
				{
					...
					wsh::Assembly const& assembly = _context.compiledContract(*contract);
					CompilerUtils(_context).fetchFreeMemoryPointer();
					// pushes size
					auto subroutine = _context.addSubroutine(make_shared<wsh::Assembly>(assembly));
		...
		case FunctionType::Kind::Transfer:
			_functionCall.expression().accept(*this);
			// Provide the gas stipend manually at first because we may send zero wise.
			// Will be zeroed if we send more than zero wise.
			m_context << u256(wsh::GasCosts::callStipend);
			arguments.front()->accept(*this);
			utils().convertType(
...
void ExpressionCompiler::appendAndOrOperatorCode(BinaryOperation const& _binaryOperation)
{
	...
	if (c_op == Token::And)
		m_context << Instruction::ISZERO;
	wsh::AssemblyItem endLabel = m_context.appendConditionalJump();
	m_context << Instruction::POP;
...
)
{
	...
	else
	{
		...
		u256 gasNeededByCaller = wsh::GasCosts::callGas + 10;
		if (_functionType.valueSet())
			gasNeededByCaller += wsh::GasCosts::callValueTransferGas;
		if (!existenceChecked)
			gasNeededByCaller += wsh::GasCosts::callNewAccountGas; // we never know
