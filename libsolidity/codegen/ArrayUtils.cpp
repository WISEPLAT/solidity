...
void ArrayUtils::copyArrayToStorage(ArrayType const& _targetType, ArrayType const& _sourceType) const
{
	...
	m_context.callLowLevelFunction(
		...
		[=](CompilerContext& _context)
		{
			...
			wsh::AssemblyItem copyLoopEndWithoutByteOffset = _context.newTag();
			...
			if (_targetType.isByteArray())
			{
				...
				wsh::AssemblyItem longByteArray = _context.appendConditionalJump();
			...
			wsh::AssemblyItem copyLoopStart = _context.newTag();
			_context << copyLoopStart;
			// check for loop condition
			_context
				<< dupInstruction(3 + byteOffsetSize) << dupInstruction(2 + byteOffsetSize)
				<< Instruction::GT << Instruction::ISZERO;
			wsh::AssemblyItem copyLoopEnd = _context.appendConditionalJump();
			// stack: target_ref target_data_end source_data_pos target_data_pos source_data_end [target_byte_offset] [source_byte_offset]
			// copy
			...
			if (haveByteOffsetTarget)
			{
				...
				wsh::AssemblyItem copyCleanupLoopEnd = _context.appendConditionalJump();
...
void ArrayUtils::copyArrayToMemory(ArrayType const& _sourceType, bool _padToWordBoundaries) const
{
	...
	else if (_sourceType.location() == DataLocation::Memory)
	{
		...
		if (paddingNeeded)
		{
			...
			wsh::AssemblyItem skip = m_context.newTag();
			if (_sourceType.isDynamicallySized())
			{
	...
	else
	{
		...
		m_context << Instruction::DUP1 << Instruction::ISZERO;
		wsh::AssemblyItem loopEnd = m_context.appendConditionalJump();
		// Special case for tightly-stored byte arrays
		if (_sourceType.isByteArray())
		{
			// stack here: memory_offset storage_offset length
			m_context << Instruction::DUP1 << u256(31) << Instruction::LT;
			wsh::AssemblyItem longByteArray = m_context.appendConditionalJump();
...
void ArrayUtils::clearDynamicArray(ArrayType const& _type) const
{
	...
	wsh::AssemblyItem endTag = m_context.newTag();
...
void ArrayUtils::resizeDynamicArray(ArrayType const& _typeIn) const
{
	...
	m_context.callLowLevelFunction(
		...
		[type](CompilerContext& _context)
		{
			...
			unsigned stackHeightStart = _context.stackHeight();
			wsh::AssemblyItem resizeEnd = _context.newTag();
			...
			if (_type.isByteArray())
			{
				wsh::AssemblyItem regularPath = _context.newTag();
				...
				wsh::AssemblyItem currentIsLong = _context.appendConditionalJump();
				_context << Instruction::DUP3 << u256(31) << Instruction::LT;
				wsh::AssemblyItem newIsLong = _context.appendConditionalJump();

				// Here: short -> short

				// Compute 1 << (256 - 8 * new_size)
				wsh::AssemblyItem shortToShort = _context.newTag();
...
void ArrayUtils::clearStorageLoop(TypePointer const& _type) const
{
	m_context.callLowLevelFunction(
		...
		[_type](CompilerContext& _context)
		{
			...
			wsh::AssemblyItem returnTag = _context.pushNewTag();
			_context << Instruction::SWAP2 << Instruction::SWAP1;

			// stack: <return tag> end_pos pos
			wsh::AssemblyItem loopStart = _context.appendJumpToNew();
			_context << loopStart;
			// check for loop condition
			_context << Instruction::DUP1 << Instruction::DUP3
					   << Instruction::GT << Instruction::ISZERO;
			wsh::AssemblyItem zeroLoopEnd = _context.newTag();
