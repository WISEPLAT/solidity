...
void ContractCompiler::compileContract(
	...
	std::map<const ContractDefinition*, wsh::Assembly const*> const& _contracts
)
{
...
size_t ContractCompiler::compileClone(
	ContractDefinition const& _contract,
	map<ContractDefinition const*, wsh::Assembly const*> const& _contracts
)
{
	initializeContext(_contract, _contracts);
...
void ContractCompiler::initializeContext(
	ContractDefinition const& _contract,
	map<ContractDefinition const*, wsh::Assembly const*> const& _compiledContracts
)
{
...
{
	// Throw if function is not payable but call contained wise.
...
size_t ContractCompiler::packIntoContractCreator(ContractDefinition const& _contract)
{
	...
	wsh::AssemblyItem deployRoutine = m_context.appendJumpToNew();
...
void ContractCompiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	map<FixedHash<4>, const wsh::AssemblyItem> callDataUnpackerEntryPoints;

	FunctionDefinition const* fallback = _contract.fallbackFunction();
	wsh::AssemblyItem notFound = m_context.newTag();
	...
	for (auto const& it: interfaceFunctions)
	{
		...
		wsh::AssemblyItem returnTag = m_context.pushNewTag();
...
bool ContractCompiler::visit(FunctionDefinition const& _function)
{
	...
	if (!_function.isConstructor() && !_function.isFallback())
		m_context.appendJump(wsh::AssemblyItem::JumpType::OutOfFunction);
	return false;
}
...
bool ContractCompiler::visit(IfStatement const& _ifStatement)
{
	...
	wsh::AssemblyItem falseTag = m_context.appendConditionalJump();
	wsh::AssemblyItem endTag = falseTag;
...
bool ContractCompiler::visit(WhileStatement const& _whileStatement)
{
	...
	wsh::AssemblyItem loopStart = m_context.newTag();
	wsh::AssemblyItem loopEnd = m_context.newTag();
...
bool ContractCompiler::visit(ForStatement const& _forStatement)
{
	...
	wsh::AssemblyItem loopStart = m_context.newTag();
	wsh::AssemblyItem loopEnd = m_context.newTag();
	wsh::AssemblyItem loopNext = m_context.newTag();
...
wsh::AssemblyPointer ContractCompiler::cloneRuntime()
{
	wsh::Assembly a;
	...
	a << u256(wsh::GasCosts::callGas + 10) << Instruction::GAS << Instruction::SUB;
	a << Instruction::DELEGATECALL;
	//Propagate error condition (if DELEGATECALL pushes 0 on stack).
	a << Instruction::ISZERO;
	a << Instruction::ISZERO;
	wsh::AssemblyItem afterTag = a.appendJumpI().tag();
	a << Instruction::INVALID << afterTag;
	//@todo adjust for larger return values, make this dynamic.
	a << u256(0x20) << u256(0) << Instruction::RETURN;
	return make_shared<wsh::Assembly>(a);
}
