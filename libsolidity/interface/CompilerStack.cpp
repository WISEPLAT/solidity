...
bool CompilerStack::compile()
{
	...
	map<ContractDefinition const*, wsh::Assembly const*> compiledContracts;
	for (Source const* source: m_sourceOrder)
		for (ASTPointer<ASTNode> const& node: source->ast->nodes())
...
wsh::AssemblyItems const* CompilerStack::assemblyItems(string const& _contractName) const
{
	Contract const& currentContract = contract(_contractName);
	return currentContract.compiler ? &contract(_contractName).compiler->assemblyItems() : nullptr;
}

wsh::AssemblyItems const* CompilerStack::runtimeAssemblyItems(string const& _contractName) const
{
	Contract const& currentContract = contract(_contractName);
...
wsh::LinkerObject const& CompilerStack::object(string const& _contractName) const
{
	return contract(_contractName).object;
}

wsh::LinkerObject const& CompilerStack::runtimeObject(string const& _contractName) const
{
	return contract(_contractName).runtimeObject;
}

wsh::LinkerObject const& CompilerStack::cloneObject(string const& _contractName) const
{
...
) const
{
	...
	if (!compiler)
		return 0;
	wsh::AssemblyItem tag = compiler->functionEntryLabel(_function);
	if (tag.type() == wsh::UndefinedItem)
		return 0;
	wsh::AssemblyItems const& items = compiler->runtimeAssemblyItems();
	for (size_t i = 0; i < items.size(); ++i)
		if (items.at(i).type() == wsh::Tag && items.at(i).data() == tag.data())
			return i;
	return 0;
}
...
void CompilerStack::compileContract(
	...
	map<ContractDefinition const*, wsh::Assembly const*>& _compiledContracts
)
{
	if (
	...
	catch(wsh::OptimizerException const&)
	{
		solAssert(false, "Assembly optimizer exception for bytecode");
	}
	catch(wsh::AssemblyException const&)
	{
...
string CompilerStack::computeSourceMapping(wsh::AssemblyItems const& _items) const
{
	string ret;
	...
	for (auto const& item: _items)
	{
		...
		int sourceIndex =
			...
			-1;
		char jump = '-';
		if (item.getJumpType() == wsh::AssemblyItem::JumpType::IntoFunction)
			jump = 'i';
		else if (item.getJumpType() == wsh::AssemblyItem::JumpType::OutOfFunction)
			jump = 'o';
...
Json::Value CompilerStack::gasEstimates(string const& _contractName) const
{
	...
	if (wsh::AssemblyItems const* items = assemblyItems(_contractName))
	{
		Gas executionGas = GasEstimator::functionalEstimation(*items);
		u256 bytecodeSize(runtimeObject(_contractName).bytecode.size());
		Gas codeDepositGas = bytecodeSize * wsh::GasCosts::createDataGas;
	...
	if (wsh::AssemblyItems const* items = runtimeAssemblyItems(_contractName))
	{
		/// External functions
