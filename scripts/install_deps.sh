...
# wearable and SBC platforms covered by doublethink and WshEmbedded would
...
#------------------------------------------------------------------------------
    ...
    Darwin)
        ...
        if [ "$CI" = true ]; then
            ...
            brew install cpp-wiseplat
            brew linkapps cpp-wiseplat
        else
            brew upgrade
        fi
...
#------------------------------------------------------------------------------
    ...
    FreeBSD)
        ...
        echo "Drop us a message at https://gitter.im/wiseplat/solidity-dev."
        exit 1
        ;;
...
#------------------------------------------------------------------------------
            ...
            Arch)
                ...
                # Also adding wiseplat-git to allow for testing with the `wsh` client
                sudo pacman -Sy \
                    base-devel \
                    boost \
                    cmake \
            ...
            Debian)
                ...
                case $(lsb_release -cs) in
                    wheezy)
                        ...
                        echo "Drop us a message at https://gitter.im/wiseplat/solidity-dev."
                        echo "See also https://github.com/wiseplat/webthree-umbrella/issues/495 where we are working through Alpine support."
                        exit 1
                        ;;
                    jessie)
                        #jessie
...
#------------------------------------------------------------------------------
            ...
            "openSUSE project")
                ...
                echo "See https://github.com/wiseplat/webthree-umbrella/issues/552."
                exit 1
                ;;
            ...
            Ubuntu)
                ...
                case $(lsb_release -cs) in
                    ...
                    *)
                        ...
                        echo "ERROR - This might not work, but we are trying anyway."
                        echo "Please drop us a message at https://gitter.im/wiseplat/solidity-dev."
                ...
                if [ "$CI" = true ]; then
                    ...
                    # Install 'wsh', for use in the Solidity Tests-over-IPC.
                    # We will not use this 'wsh', but its dependencies
                    sudo add-apt-repository -y ppa:wiseplat/wiseplat
                    sudo add-apt-repository -y ppa:wiseplat/wiseplat-dev
                    sudo apt-get -y update
                    sudo apt-get -y install wsh
                fi
                ;;
