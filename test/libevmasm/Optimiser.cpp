...
namespace
{
	...
	wsh::KnownState createInitialState(AssemblyItems const& _input)
	{
		wsh::KnownState state;
		for (auto const& item: addDummyLocations(_input))
			state.feedItem(item, true);
		return state;
	}

	AssemblyItems CSE(AssemblyItems const& _input, wsh::KnownState const& _state = wsh::KnownState())
	{
		AssemblyItems input = addDummyLocations(_input);

		wsh::CommonSubexpressionEliminator cse(_state);
	...
	void checkCSE(
		...
		KnownState const& _state = wsh::KnownState()
	)
	{
...
{
	wsh::KnownState state;
	wsh::CommonSubexpressionEliminator cse(state);
	AssemblyItems input{
...
{
	wsh::KnownState state = createInitialState(AssemblyItems{
		u256(0x12),
		u256(0x20),
		Instruction::ADD
	});
...
{
	wsh::KnownState state = createInitialState(AssemblyItems{Instruction::DUP1});
	AssemblyItems input{
		Instruction::EQ
	};
