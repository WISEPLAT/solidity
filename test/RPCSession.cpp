...
*/
/// @file RPCSession.cpp
/// Low-level IPC communication between the test framework and the Wiseplat node.
...
string RPCSession::wsh_getCode(string const& _address, string const& _blockNumber)
{
	return rpcCall("wsh_getCode", { quote(_address), quote(_blockNumber) }).asString();
}

Json::Value RPCSession::wsh_getBlockByNumber(string const& _blockNumber, bool _fullObjects)
{
	// NOTE: to_string() converts bool to 0 or 1
	return rpcCall("wsh_getBlockByNumber", { quote(_blockNumber), _fullObjects ? "true" : "false" });
}

RPCSession::TransactionReceipt RPCSession::wsh_getTransactionReceipt(string const& _transactionHash)
{
	TransactionReceipt receipt;
	Json::Value const result = rpcCall("wsh_getTransactionReceipt", { quote(_transactionHash) });
...
string RPCSession::wsh_sendTransaction(TransactionData const& _td)
{
	return rpcCall("wsh_sendTransaction", { _td.toJson() }).asString();
}

string RPCSession::wsh_call(TransactionData const& _td, string const& _blockNumber)
{
	return rpcCall("wsh_call", { _td.toJson(), quote(_blockNumber) }).asString();
}

string RPCSession::wsh_sendTransaction(string const& _transaction)
{
	return rpcCall("wsh_sendTransaction", { _transaction }).asString();
}

string RPCSession::wsh_getBalance(string const& _address, string const& _blockNumber)
{
	string address = (_address.length() == 20) ? "0x" + _address : _address;
	return rpcCall("wsh_getBalance", { quote(address), quote(_blockNumber) }).asString();
}

string RPCSession::wsh_getStorageRoot(string const& _address, string const& _blockNumber)
{
	string address = (_address.length() == 20) ? "0x" + _address : _address;
	return rpcCall("wsh_getStorageRoot", { quote(address), quote(_blockNumber) }).asString();
}
...
{
	u256 startBlock = fromBigEndian<u256>(fromHex(rpcCall("wsh_blockNumber").asString()));
	...
	for (; ; ++tries)
	{
		...
		if (fromBigEndian<u256>(fromHex(rpcCall("wsh_blockNumber").asString())) >= startBlock + _number)
			break;
		else
			sleepTime *= 2;
	}
