...
contract ico is safeMath {
    ...
    function setICOWshPrice(uint256 value) external {
        /*
    ...
    function closeICO() external {
        /*
            ...
            Wises which are situated in this contract will be sent to the address of the fundation.
        */
        require( msg.sender == owner );
        require( block.number > icoDelay );
        require( ! closed );
    ...
    function receiveFunds() external {
        ...
        */
        require( aborted );
        require( brought[msg.sender].wsh > 0 );
        uint256 _val = brought[msg.sender].wsh * 90 / 100;
    ...
    function buy(address beneficiaryAddress, address affilateAddress) payable returns (bool success) {
        /*
            ...
            If there is not at least 0.2 wise balance on the beneficiaryAddress then the amount of the wise which was intended for the purchase will be reduced by 0.2 and that will be sent to the address of the beneficiary.
        ...
        */
        ...
        if ( beneficiaryAddress.balance < 0.2 wise ) {
            require( beneficiaryAddress.send(0.2 wise) );
            _value = safeSub(_value, 0.2 wise);
        }
        var _reward = getIcoReward(_value);
        require( _reward > 0 );
        require( token(tokenAddr).mint(beneficiaryAddress, _reward) );
        brought[beneficiaryAddress].wsh = safeAdd(brought[beneficiaryAddress].wsh, _value);
        ...
        uint256 extra;
        if ( affilateAddress != 0x00 && ( brought[affilateAddress].wsh > 0 || interestDB[affilateAddress][0].amount > 0 ) ) {
    ...
    function getIcoReward(uint256 value) public constant returns (uint256 reward) {
        /*
            ...
            @value The amount of wise for the purchase
        ...
        */
        reward = (value * 1e6 * icoExchangeRate / icoExchangeRateM / 1 wise) * (ICObonus() + 100) / 100;
