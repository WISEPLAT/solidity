...
BOOST_AUTO_TEST_CASE(blockchain)
{
	...
	BOOST_CHECK(m_rpc.rpcCall("miner_setWisebase", {"\"0x1212121212121212121212121212121212121212\""}).asBool() == true);
...
BOOST_AUTO_TEST_CASE(send_wise)
{
	char const* sourceCode = R"(
		contract test {
			function test() payable {}
...
BOOST_AUTO_TEST_CASE(transfer_wise)
{
	char const* sourceCode = R"(
		contract A {
			function A() payable {}
...
BOOST_AUTO_TEST_CASE(contracts_as_addresses)
{
	char const* sourceCode = R"(
		contract helper {
			function() payable { } // can receive wise
		}
		contract test {
			helper h;
...
// Disabled until https://github.com/wiseplat/solidity/issues/715 is implemented
...
BOOST_AUTO_TEST_CASE(send_zero_wise)
{
	// Sending zero wise to a contract should still invoke the fallback function
...
BOOST_AUTO_TEST_CASE(reject_wise_sent_to_library)
{
	char const* sourceCode = R"(
		library lib {}
		contract c {
	...
	char const* sourceCode = R"(
		contract C {
			modifier tryCircumvent {
				if (false) _; // avoid the function, we should still not accept wise
			}
