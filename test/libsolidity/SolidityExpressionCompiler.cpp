...
)
{
	...
	for (ASTPointer<ASTNode> const& node: sourceUnit->nodes())
		if (ContractDefinition* contract = dynamic_cast<ContractDefinition*>(node.get()))
		{
			...
			// debug
			// cout << wsh::disassemble(instructions) << endl;
			return instructions;
		}
...
BOOST_AUTO_TEST_CASE(int_with_wei_wise_subdenomination)
{
	char const* sourceCode = R"(
		contract test {
			function test () {
...
BOOST_AUTO_TEST_CASE(int_with_szabo_wise_subdenomination)
{
	char const* sourceCode = R"(
		contract test {
			function test () {
...
BOOST_AUTO_TEST_CASE(int_with_finney_wise_subdenomination)
{
	char const* sourceCode = R"(
		contract test {
			function test ()
...
BOOST_AUTO_TEST_CASE(int_with_wise_wise_subdenomination)
{
	char const* sourceCode = R"(
		contract test {
			function test () {
