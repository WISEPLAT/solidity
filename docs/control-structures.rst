...
::
    ...
    contract C {
        ...
        function createAndEndowD(uint arg, uint amount) payable {
            // Send wise along with the creation
            D newD = (new D).value(amount)(arg);
        }
    }

As seen in the example, it is possible to forward Wise while creating
...
#. If your contract receives Wise via a public function without ``payable`` modifier (including the constructor and the fallback function).
#. If your contract receives Wise via a public getter function.
#. If a ``.transfer()`` fails.
