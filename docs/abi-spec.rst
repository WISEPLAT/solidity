...
The Application Binary Interface is the standard way to interact with contracts in the Wiseplat ecosystem, both
...
This specification does not address contracts whose interface is dynamic or otherwise known only at run-time. Should these cases become important they can be adequately handled as facilities built within the Wiseplat ecosystem.
...
Events are an abstraction of the Wiseplat logging/event-watching protocol. Log entries provide the contract's address, a series of up to four topics and some arbitrary length binary data. Events leverage the existing function ABI in order to interpret this (together with an interface spec) as a properly typed structure.
...
- ``address``: the address of the contract (intrinsically provided by Wiseplat);
...
- ``payable``: ``true`` if function accepts wise, defaults to ``false``;
...
Sending non-zero wise to non-payable function will throw. Don't do it.
