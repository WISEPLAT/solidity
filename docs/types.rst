...
``address``: Holds a 20 byte value (size of an Wiseplat address). Address types also have members and serve as a base for all contracts.
...
and to send Wise (in units of wei) to an address using the ``transfer`` function:
...
.. warning::
    ...
    to make safe Wise transfers, always check the return value of ``send``, use ``transfer`` or even better:
...
Similarly, the supplied Wise value can be controlled too::

    nameReg.call.value(1 wise)("register", "MyName");

Lastly, these modifiers can be combined. Their order does not matter::

    nameReg.call.gas(1000000).value(1 wise)("register", "MyName");
...
.. note::
    The mixed-case address checksum format is defined in `EIP-55 <https://github.com/wiseplat/EIPs/blob/master/EIPS/eip-55.md>`_.
...
.. note::
  ...
  For an example, see `iterable mapping <https://github.com/wiseplat/dapp-bin/blob/master/library/iterable_mapping.sol>`_.
