...
Contracts can be created "from outside" via Wiseplat transactions or from within Solidity contracts.

IDEs, such as `Remix <https://remix.wiseplat.org/>`_, make the creation process seamless using UI elements.

Creating contracts programatically on Wiseplat is best done via using the JavaScript API `web3.js <https://github.com/wiseplat/web3.js>`_.
As of today it has a method called `web3.wsh.Contract <https://web3js.readthedocs.io/en/1.0/web3-wsh-contract.html#new-contract>`_
...
::
    ...
    contract Register is priced, owned {
        ...
        // automatically reject all Wise sent to it.
...
Wise (without data). Additionally, in order to receive Wise, the fallback function
must be marked ``payable``. If no such function exists, the contract cannot receive
Wise through regular transactions.
...
.. warning::
    Contracts that receive Wise directly (without a function call, i.e. using ``send`` or ``transfer``)
    but do not define a fallback function
    throw an exception, sending back the Wise (this was different
    before Solidity v0.4.0). So if you want your contract to receive Wise,
    you have to implement a fallback function.

.. warning::
    A contract without a payable fallback function can receive Wise as a recipient of a `coinbase transaction` (aka `miner block reward`)
    or as a destination of a ``selfdestruct``.

    A contract cannot react to such Wise transfers and thus also cannot reject them. This is a design choice of the EVM and Solidity cannot work around it.
...
::
    ...
    contract Test {
        ...
        // Sending Wise to this contract will cause an exception,
    ...
    // This contract keeps all Wise sent to it with no way
    // to get it back.
    contract Sink {
        function() payable { }
    }
    ...
    contract Caller {
        function callTest(Test test) {
            ...
            // if someone sends wise to that contract,
...
::
    ...
    var ClientReceipt = web3.wsh.contract(abi);
...
- `Javascript documentation <https://github.com/wiseplat/wiki/wiki/JavaScript-API#contract-events>`_
