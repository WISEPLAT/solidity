...
.. index:: wei, finney, szabo, wise

Wise Units
===========

A literal number can take a suffix of ``wei``, ``finney``, ``szabo`` or ``wise`` to convert between the subdenominations of Wise, where Wise currency numbers without a postfix are assumed to be Wei, e.g. ``2 wise == 2000 finney`` evaluates to ``true``.
...
``keccak256(...) returns (bytes32)``:
    compute the Wiseplat-SHA-3 (Keccak-256) hash of the :ref:`(tightly packed) arguments <abi_packed_mode>`
...
``ecrecover(bytes32 hash, uint8 v, bytes32 r, bytes32 s) returns (address)``:
    ...
    (`example usage <https://wiseplat.stackexchange.com/q/1777/222>`_)
