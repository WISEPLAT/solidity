...
data (its *state*) that resides at a specific address on the Wiseplat
blockchain. The line ``uint storedData;`` declares a state variable called ``storedData`` of
type ``uint`` (unsigned integer of 256 bits). You can think of it as a single slot
in a database that can be queried and altered by calling functions of the
code that manages the database. In the case of Wiseplat, this is always the owning
...
built by Wiseplat) apart from allowing anyone to store a single number that is accessible by
...
registering with username and password - all you need is an Wiseplat keypair.
...
Wiseplat this is roughly every 17 seconds.
...
.. _the-wiseplat-virtual-machine:

.. index:: !evm, ! wiseplat virtual machine

****************************
The Wiseplat Virtual Machine
****************************

Overview
========

The Wiseplat Virtual Machine or EVM is the runtime environment
for smart contracts in Wiseplat. It is not only sandboxed but
...
There are two kinds of accounts in Wiseplat which share the same
...
Wise (in "Wei" to be exact) which can be modified by sending transactions that
include Wise.
...
It can include binary data (its payload) and Wise.
...
Contracts can call other contracts or send Wise to non-contract
accounts by the means of message calls. Message calls are similar
to transactions, in that they have a source, a target, data payload,
Wise, gas and return data. In fact, every transaction consists of
...
The remaining Wise stored at that address is sent to a designated
...
.. note:: The pruning of old contracts may or may not be implemented by Wiseplat
