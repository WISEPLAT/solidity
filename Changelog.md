 ...
 * Introduce ``.transfer(value)`` for sending Wise.
...
to explicitly specify if functions can receive wise via the ``payable``
modifier. Furthermore, more situations cause exceptions to be thrown.

Minimal changes to be made for upgrade:
 - Add ``payable`` to all functions that want to receive Wise
 ...
 * Functions that want to receive Wise have to specify the
   new ``payable`` modifier (otherwise they throw).
 * Contracts that want to receive Wise with a plain "send"
 ...
 * Moved (and reworked) standard library contracts from inside the compiler to github.com/wiseplat/solidity/std
 ...
 * Version string includes libevmasm/libwiseplat's version (contains the optimizer).
